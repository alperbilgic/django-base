...
from .errors import GoogleError, InAppPyError, InAppPyValidationError
...
class GooglePlayValidator:
    ...
    def _validate_signature(self, receipt: str, signature: str) -> bool:
        try:
            ...
            return bool(rsa.verify(receipt.encode(), sig, self.public_key))
...
class GooglePlayVerifier:
    ...
    def __init__(
        self,
        bundle_id: str,
        play_console_credentials: Union[str, dict],
        http_timeout: int = 15,
    ) -> None:
        """
        Arguments:
    ...
    def _create_credentials(play_console_credentials: Union[str, dict], scope_str: str):
        ...
        if isinstance(play_console_credentials, str):
            if not os.path.exists(play_console_credentials):
                raise InAppPyError(
                    f"Google play console credentials file does not exist: {play_console_credentials}"
                )
    ...
    def check_purchase_buyable(
        self, purchase_token: str, product_sku: str, service
    ) -> dict:
        try:
            purchases = service.purchases()
            buyables = purchases.buyables()
            buyables_get = buyables.get(
                packageName=self.bundle_id, productId=product_sku, token=purchase_token
            )
            result = buyables_get.execute(http=self.http)
            return result
        except HttpError as e:
            if e.resp.status == 400:
                raise GoogleError(e.resp.reason, repr(e))
    ...
    def verify(
        ...
        if is_subscription:
            ...
            result = self.check_purchase_buyable(purchase_token, product_sku, service)
